...
/** A utility object for interacting with a Prolog engine using the `tuProlog` library. This object provides Scala-friendly functions for creating Prolog engines,
  * extracting terms, and converting data to Prolog terms.
  */
object ScalaPrologEngine:

  /** Implicit conversion from `String` to Prolog `Term`. Allows easy creation of Prolog terms from strings.
    */
  given Conversion[String, Term] = Term.createTerm(_)

  /** Implicit conversion from `Seq` to Prolog `Term`. Converts a Scala sequence into a Prolog list term.
    */
  ...
  /** Extracts a specific term from a Prolog structure at a given index.
    *
    * @param t
    *   The Prolog term, which should be a `Struct`.
    * @param i
    *   The index of the argument to extract.
    * @return
    *   The extracted `Term` at the specified index.
    */
  def extractTerm(t: Term, i: Integer): Term =
    t.asInstanceOf[Struct].getArg(i).getTerm

  /** Creates a Prolog engine and sets its theory (rules and facts) based on the provided clauses. Returns a function that takes a goal as input and produces a lazy list
    * of solutions.
    *
    * @param clauses
    *   The Prolog clauses (rules and facts) to be set as the theory.
    * @return
    *   A function that, given a goal, returns a `LazyList` of solutions.
    */
      ...
      new Iterable[Term] {
        override def iterator: Iterator[Term] = new Iterator[Term] {
          ...
          /** Retrieves the current solution and moves to the next one.
            *
            * @return
            *   The current `Term` solution.
            * @throws NoSuchElementException
            *   if no more solutions are available.
            */
