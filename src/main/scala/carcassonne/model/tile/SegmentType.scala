...
object SegmentType:
  /**
   * Implicit JSON format for `SegmentType`.
   * Provides methods to convert `SegmentType` to and from JSON.
   */
  given Format[SegmentType] with
     ...
     * @return A `JsResult` containing the `SegmentType` if successful, or an error message if not.
     */
     ...
     *
     * @param segmentType The `SegmentType` to write.
     * @return A `JsValue` representing the `SegmentType`.
     */
   ...
   * @return A `Try` containing the `SegmentType` if successful, or an exception if not.
   */
