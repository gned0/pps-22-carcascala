...
/** Represents a deck of tiles in the Carcassonne game.
  *
  * This trait defines the basic functionality of a tile deck, which is to draw tiles. Note that because the only condition that ends a game is the deck being empty, the
  * draw functionality is also used to handle the end of the game.
  */
trait TileDeck:
...
/** Companion object for TileDeck.
  *
  * This object provides factory methods to create TileDeck instances and handles the loading of tiles from a configuration file.
  */
object TileDeck:
  ...
  /** Creates a new TileDeck instance.
    ...
    *   The path to the JSON configuration file for the deck. Defaults to "src/main/resources/deck.json", which is the standard deck configuration for the game as
