...
trait SubjectGameMatchView:
  ...
  /** Notifies all observers of a follower placement.
    *
    * @return
    *   a function that takes a position, a tile segment, and a player, and notifies all observers
    */
  def notifyFollowerPlacement: (Position, TileSegment, Player) => Unit =
    (position, segment, player) => observers.foreach(_.placeFollower(position, segment, player))
  ...
  /** Attempts to notify all observers of a follower placement, returning a Try.
    *
    * @param position
    *   the position where the follower is being placed
    * @param segment
