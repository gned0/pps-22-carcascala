...
/**
 * Utility object providing methods to stringify game-related data in a format compatible with Prolog.
 * It helps convert game elements like tiles and connected features into Prolog-readable strings.
 */
object ScalaPrologUtils {
   ...
   * `tile(x, y, [(segment, type), ...])`
   *
   * @return A function that takes a `Position` and a `GameTile` and returns a formatted string.
   */
   ...
   *
   * The resulting string will be in the format:
   * `feature_completed([(x, y, segment), ...])`
   *
   * @param feature           The feature name (e.g., "city", "road").
   * @param connectedFeatures A set of tuples representing positions and corresponding tile segments.
   * @return                  A formatted string representing the completed feature and its associated segments.
   */
