...
class ScoreAdjacencyPositions:
  /** Gets the adjacent tiles and segments for a given position and segment.
    ...
    * @return
    *   a list of adjacent positions and segments
    */
  def getAdjacentCityTilesAndSegments(position: Position, segment: TileSegment): List[(Option[Position], TileSegment)] =
    segment match
  ...
  /** Gets the adjacent road tiles and segments for a given position and segment.
    ...
    *   a list of adjacent positions and segments
    */
  def getAdjacentRoadTilesAndSegments(position: Position, segment: TileSegment): List[(Option[Position], TileSegment)] =
    segment match
      case N => List((Some(Position(position.x, position.y - 1)), S))
      case S => List((Some(Position(position.x, position.y + 1)), N))
      case E => List((Some(Position(position.x + 1, position.y)), W))
  ...
  /** Gets the adjacent field segments for a given position and segment.
    ...
    *   a list of adjacent positions and segments
    */
  def getAdjacentFieldSegments(position: Position, segment: TileSegment): List[(Option[Position], TileSegment)] =
    segment match
      case N =>
        List(
          (Some(position), NW),
