  ...
  private def createPlayerSetupPane(): VBox = {
    ...
    val playerCountComboBox = new ComboBox[Int](2 to 5) {
      ...
      onAction = _ => {
        ...
        playerNames.zipWithIndex.foreach { case (textField, index) =>
          textField.visible = index < selectedCount
        }
      }
    }
...
1. This shows the number of tiles still remaining on the deck;
2. This is tile drawn from the randomized deck of tiles, which the player can place on the board based on the borders of the tile drawn, and the borders of the tiles placed on the board, which have to match;
3. These buttons are used to, respectively from top left to bottom right, rotate the drawn counter-clockwise, clockwise, and/or skip the tile or follower placement, that is if the player wants to or is forced because either they don't have a place to to place the tile, or they don't have any followers;
4. This is the scoreboard, very simply show the score of each player;
5. This is the button used to end the game early, without showing the final score of the players;
6. This is the main board of the game, which can be zoomed in or out using the mouse wheel, and panned around using the mouse right button. Only the lighter background of the board is pannable, since the growth of the board itself is variable, the darker background is just used to blend better the environment;
7. This is the starter tile which is already placed on the board when starting a match.
8. This is a placeholder to show the possible placement of the tile that has been drawn, it's up to the player to decide to provide the correct orientation of the borders through the buttons on the right menu;
9. This is the what it looks like when trying to place a tile on the board, if the tile drawn matches the border/s of the tile/s already placed on the board, then the tile will be placed, otherwise it won't be.
10. This button is used to recenter the camera to the origin of the board, in case the player wants to reset the view after panning and/or zooming.
...
Below two more images are shown to give a visual representation of the follower placement: """
      ...
      new Text():
        ...
        wrappingWidth <== contentBox.width - 20
        text =
          s"""As we can see, the player can place the follower in any of the feature present on the tile, and later on at the end of every turn and at the end of the match, the score will be calculated and added for each completed feature as described in the basic Carcassone rules."""
      ,
      new Text():
        alignment = Center
