...
/** Represents the view for the game match board in the Carcassonne game.
  *
  * @param gameEndedSwitchView
  *   A function to switch the view when the game ends.
  */
class GameMatchBoardView(gameEndedSwitchView: () => Unit) extends GridPane with ObserverGameMenuView with SubjectGameMatchView with ObserverGameMatchBoard:
  ...
  /** Places a tile at the specified position on the board.
    *
    * @param position
    *   The position where the tile should be placed.
    * @param tileGraphicElement
    *   The graphical representation of the tile.
    */
  private def placeTile(position: Position, tileGraphicElement: Node): Unit =
    this.getChildren.removeIf(node => getColumnIndex(node) == position.x && getRowIndex(node) == position.y)
    this.add(tileGraphicElement, position.x, position.y)

  /** Creates new placeholder tiles around the specified position.
    *
    * @param position
    *   The position around which new placeholders should be created.
    */
  private def createNewPlaceholders(position: Position): Unit =
    Seq(position.x - 1, position.x + 1).foreach(posX =>
      if !this.getChildren.exists(node => getColumnIndex(node) == posX && getRowIndex(node) == position.y) then createPlaceholderTile(Position(posX, position.y))
    )
    Seq(position.y - 1, position.y + 1).foreach(posY =>
      if !this.getChildren.exists(node => getColumnIndex(node) == position.x && getRowIndex(node) == posY) then createPlaceholderTile(Position(position.x, posY))
    )

  /** Creates a placeholder tile at the specified position.
    *
    * @param position
    *   The position where the placeholder tile should be created.
    * @return
    *   The created placeholder tile.
    */
  private def createPlaceholderTile(position: Position): Region =
    new Region:
      ...
      onMouseClicked = (event: MouseEvent) => if event.button == MouseButton.Primary then notifyTilePlacementAttempt(getDrawnTile._1, position)
      add(this, position.x, position.y)

  /** Removes the follower grid pane at the specified position.
    *
    * @param position
    *   The position from which the follower grid pane should be removed.
    */
  private def removeFollowerGridPane(position: Position): Unit =
    this.getChildrenUnmodifiable.toArray
      .collectFirst(child =>
        child match
          case child: javafx.scene.Node
              if getColumnIndex(child) == position.x &&
                getRowIndex(child) == position.y =>
            child
      )
      .collect(stackPane =>
        stackPane match
  ...
  /** Updates the board when a tile is placed.
    *
    * @param isTilePlaced
    *   Indicates whether the tile was successfully placed.
    * @param position
    *   The position where the tile was placed.
    */
  override def isTilePlaced(isTilePlaced: Boolean, position: Position): Unit =
  ...
  /** Displays the available follower positions on the board.
    *
    * @param availSegments
    *   The list of available tile segments for placing followers.
    * @param position
